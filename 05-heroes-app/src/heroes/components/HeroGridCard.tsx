...
export const HeroGridCard = ({ hero }: Props) => {
    ...
    const handleClick = () => {
        navigate(`/heroes/${hero.slug}`); // slug es url friendly para los bots de google y su indexado
    };
        ...
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64">
                <img
                    src={hero.image}
                    alt={hero.name}
                    className="object-cover w-full h-102.5 transition-all duration-500 group-hover:scale-110 absolute -top-7.5"
                    onClick={handleClick}
                />
