...
export const HeroPage = () => {
    ...
    const { idSlug = '' } = useParams(); // recuperamos el nombre del heroe de los parámetros de la url
    ...
    const {
        ...
        isError,
        error
    } = useQuery<Hero>({ // recuperamos la info del héroe de tanstack
        queryKey: ["hero-information", idSlug],
        queryFn: () => getHero(idSlug),
        retry: false // no reintenta la petición
    });

    if (isPending) {
        return (
            <div className="min-h-screen grid place-items-center">
                <p className="text-gray-600">Cargando héroe...</p>
            </div>
        );
    }

    // si el usuario modificó el parámetro de la url y el héroe no existe o hubo un error
    if (isError || !superheroData) {
        return (
            <div className="min-h-screen grid place-items-center">
                <div className="text-center space-y-2">
                    {
                        !superheroData && (
                            <p className="text-red-600 font-semibold">El héroe no existe</p>
                        )
                    }
                    {
                        isError && (
                            <p className="text-gray-600 text-sm">{(error as Error)?.message ?? "Error desconocido"}</p>
                        )
                    }
                </div>
            </div>
